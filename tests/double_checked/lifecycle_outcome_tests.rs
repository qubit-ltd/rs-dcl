...
fn test_lifecycle_outcome_non_panic_states_are_distinct() {
    let initial: LifecycleOutcome<(), io::Error, io::Error> = LifecycleOutcome::InitialConditionNotMet;
    let prepare: LifecycleOutcome<(), io::Error, io::Error> =
        LifecycleOutcome::PrepareFailed(io::Error::other("prepare"));
    let success: LifecycleOutcome<u32, io::Error, io::Error> = LifecycleOutcome::TaskSucceeded {
        value: 42,
        commit: FinalizationOutcome::Succeeded,
    };
    let second: LifecycleOutcome<(), io::Error, io::Error> = LifecycleOutcome::SecondConditionNotMet {
        rollback: FinalizationOutcome::Succeeded,
    };
    let task: LifecycleOutcome<(), io::Error, io::Error> = LifecycleOutcome::TaskFailed {
