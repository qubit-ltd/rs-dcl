...
impl Drop for CoverageToken {
    ...
    fn drop(&mut self) {
        match self.drop_behavior {
            ...
            CoverageTokenDrop::PanickingPayload => std::panic::panic_any(PanicOnDrop),
        }
    }
}
...
fn successful_coverage_task(_token: &mut CoverageToken) -> Result<u32, io::Error> {
    Ok(42)
}
...
fn panicking_coverage_task(_token: &mut CoverageToken) -> Result<u32, io::Error> {
    panic!("coverage task panic")
}

/// Verifies a propagating matrix invocation resumes its original task panic.
fn assert_original_coverage_task_panic(executor: &LifecycleDclExecutor<CoverageToken, io::Error>, lock: &NoopLock) {
    let task = panicking_coverage_task as CoverageTask;
    let panic_result = catch_unwind(AssertUnwindSafe(|| {
        let _: LifecycleOutcome<u32, io::Error, io::Error> = executor.run_with_token(lock, task);
    }));
...
fn test_run_catching_covers_all_branches_with_one_task_type() {
    ...
    let initial_false = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        initial_false.run_with_token_catching(&lock, successful_task),
    ...
    let initial_panic = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        initial_panic.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::InitialConditionCheckPanicked(panic)
    ...
    let prepare_error = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        prepare_error.run_with_token_catching(&lock, successful_task),
    ...
    let prepare_panic = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        prepare_panic.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::PreparePanicked(panic)
    ...
    let second_false = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        second_false.run_with_token_catching(&lock, successful_task),
    ...
    let task_paths = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        task_paths.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::TaskSucceeded {
            value: 42,
            commit: CapturedFinalizationOutcome::NotRequired,
        }
    ));
    assert!(matches!(
        task_paths.run_with_token_catching(&lock, failing_task),
    ...
    assert!(matches!(
        task_paths.run_with_token_catching(&lock, panicking_task),
        CapturedLifecycleOutcome::ExecutionPanicked {
            panic,
            rollback: CapturedFinalizationOutcome::Succeeded,
    ...
    let successful_commit = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        successful_commit.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::TaskSucceeded {
            value: 42,
    ...
    let failing_commit = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        failing_commit.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::TaskSucceeded {
            value: 42,
            commit: CapturedFinalizationOutcome::Failed(error),
    ...
    let panicking_commit = LifecycleDclExecutor::builder()
        .when(|| true)
        .prepare(prepare_quiet_coverage_token)
        .commit(|_| -> Result<(), io::Error> { panic!("coverage commit panic") })
        .no_rollback()
        .build();
    assert!(matches!(
        panicking_commit.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::TaskSucceeded {
            value: 42,
            commit: CapturedFinalizationOutcome::Panicked(panic),
    ...
    let failing_rollback = LifecycleDclExecutor::builder()
        ...
        .prepare(prepare_quiet_coverage_token)
        .no_commit()
        .rollback(|_, _| Err::<(), _>(io::Error::other("coverage rollback failed")))
        .build();
    assert!(matches!(
        failing_rollback.run_with_token_catching(&lock, failing_task),
        CapturedLifecycleOutcome::TaskFailed {
            error,
            rollback: CapturedFinalizationOutcome::Failed(rollback_error),
        } if error.to_string() == "coverage task failed"
    ...
    let panicking_rollback = LifecycleDclExecutor::builder()
        ...
        .prepare(prepare_quiet_coverage_token)
        .no_commit()
        .rollback(|_, _| -> Result<(), io::Error> { panic!("coverage rollback panic") })
        .build();
    assert!(matches!(
        panicking_rollback.run_with_token_catching(&lock, failing_task),
        CapturedLifecycleOutcome::TaskFailed {
            rollback: CapturedFinalizationOutcome::Panicked(panic),
            ..
    ...
    let no_rollback = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        no_rollback.run_with_token_catching(&lock, failing_task),
        CapturedLifecycleOutcome::TaskFailed {
            rollback: CapturedFinalizationOutcome::NotRequired,
            ..
        }
    ));
    ...
    let panicking_commit_drop = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        panicking_commit_drop.run_with_token_catching(&lock, successful_task),
        CapturedLifecycleOutcome::TaskSucceeded {
            value: 42,
            commit: CapturedFinalizationOutcome::Panicked(panic),
        } if panic.phase() == PanicPhase::Commit
    ...
    let panicking_rollback_drop = LifecycleDclExecutor::builder()
        ...
        .build();
    assert!(matches!(
        panicking_rollback_drop.run_with_token_catching(&lock, failing_task),
        CapturedLifecycleOutcome::TaskFailed {
            rollback: CapturedFinalizationOutcome::Panicked(panic),
            ..
        } if panic.phase() == PanicPhase::Rollback
...
fn test_run_propagating_covers_all_branches_with_one_task_type() {
    ...
    let panicking_rollback = LifecycleDclExecutor::builder()
        ...
        .prepare(prepare_quiet_coverage_token)
        .no_commit()
        .rollback(|_, _| -> Result<(), io::Error> { std::panic::panic_any(PanicOnDrop) })
        .build();
    ...
    let executor = LifecycleDclExecutor::builder()
        ...
        .rollback({
            ...
            move |token, cause| {
                ...
                saw_condition_cause.store(matches!(cause, RollbackCause::ConditionNotMet), Ordering::Relaxed);
                rollback_obtained_lock.store(lock.try_lock().is_ok(), Ordering::Relaxed);
                Ok::<(), io::Error>(())
            }
        })
        .build();
    ...
    let executor = LifecycleDclExecutor::builder()
        ...
        .rollback({
            ...
            move |_, cause| {
                ...
                *rollback_phase
                    .lock()
                    .expect("rollback phase mutex should not be poisoned") = Some(panic.phase());
                Ok::<(), io::Error>(())
            }
        })
        .build();
...
fn test_run_captures_poisoned_lock_acquisition_then_rolls_back() {
    ...
    let poison_result = catch_unwind(AssertUnwindSafe(|| {
        let _guard = poison_lock.lock().expect("standard mutex should start unpoisoned");
    ...
    let executor = LifecycleDclExecutor::builder()
        ...
        .commit({
            ...
            move |token| {
                ...
                commit_obtained_lock.store(lock.try_lock().is_ok(), Ordering::Relaxed);
                Ok::<(), io::Error>(())
            }
        })
        .no_rollback()
    ...
    let executor = LifecycleDclExecutor::builder()
        ...
        .rollback({
            ...
            move |_, cause| {
                ...
                *rollback_message
                    .lock()
                    .expect("rollback message mutex should not be poisoned") = Some(error.to_string());
                Ok::<(), io::Error>(())
            }
        })
        .build();
...
fn test_run_with_token_commits_task_updates() {
    ...
    let executor = LifecycleDclExecutor::builder()
        ...
        .commit({
            ...
            move |token| {
                *committed_token
                    .lock()
                    .expect("committed token mutex should not be poisoned") = Some(token);
                Ok::<(), io::Error>(())
            }
        })
        .no_rollback()
    ...
    let outcome = executor.run_with_token(&parking_lot::Mutex::new(()), |token| {
        token.push("task");
...
fn test_run_commit_failure_preserves_success() {
    ...
    let outcome = executor.run_catching(&parking_lot::Mutex::new(()), || Ok::<u32, io::Error>(42));
...
fn test_clone_shares_lifecycle_callbacks() {
    ...
    let outcome = cloned.run(&parking_lot::Mutex::new(()), || Ok::<u32, io::Error>(42));
...
fn test_run_captured_task_and_rollback_panics_preserves_both() {
    ...
    assert!(matches!(
        ...
        CapturedLifecycleOutcome::ExecutionPanicked {
            panic,
            rollback: CapturedFinalizationOutcome::Panicked(rollback_panic),
        } if panic.phase() == PanicPhase::Task
...
fn test_run_uncaptured_task_panic_rolls_back_then_resumes_original() {
    ...
    let panic_result = catch_unwind(AssertUnwindSafe(|| {
        let _: LifecycleOutcome<(), io::Error, io::Error> =
...
fn test_run_uncaptured_task_panic_outranks_rollback_error() {
    let executor = LifecycleDclExecutor::builder()
        ...
        .prepare(|| Ok::<(), io::Error>(()))
        .no_commit()
        .rollback(|_, _| Err::<(), _>(io::Error::other("secondary rollback error")))
        .build();
...
fn test_run_uncaptured_task_panic_outranks_rollback_error_drop_panic() {
    let test_binary = std::env::current_exe().expect("integration test binary path should be available");
...
fn test_run_uncaptured_task_panic_outranks_rollback_panic() {
    let executor = LifecycleDclExecutor::builder()
        ...
        .prepare(|| Ok::<(), io::Error>(()))
        .no_commit()
        .rollback(|_, _| -> Result<(), io::Error> { panic!("secondary rollback panic") })
        .build();
...
fn test_run_uncaptured_initial_predicate_panic_propagates() {
    ...
    let payload = panic_result.expect_err("initial predicate panic should propagate");
    assert_eq!(payload.downcast_ref::<&str>(), Some(&"initial predicate panic"));
    assert_eq!(prepare_calls.load(Ordering::Relaxed), 0);
}
...
fn test_run_concurrent_prepare_tokens_do_not_cross_calls() {
    ...
    let committed = committed.lock().expect("commit token mutex should not be poisoned");
    let rolled_back = rolled_back.lock().expect("rollback token mutex should not be poisoned");
